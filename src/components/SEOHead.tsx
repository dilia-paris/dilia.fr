...
// Lazy load schema generation to reduce initial bundle
...
const SEOHead = ({ page }: SEOHeadProps) => {
  ...
  const setMeta = useCallback((attribute: string, value: string, content: string) => {
  ...
  useEffect(() => {
    ...
    // Use requestIdleCallback for non-critical schema generation
    const generateSchemas = async () => {
      if (schemaType) {
        const schema = await generateSchema(page, pageData, schemaType, baseUrl, imageUrl, language);
    ...
    if ('requestIdleCallback' in window) {
      requestIdleCallback(() => generateSchemas());
    } else {
      setTimeout(generateSchemas, 100);
    }
  ...
  }, [page, pageData, language, siteName, setMeta]);
